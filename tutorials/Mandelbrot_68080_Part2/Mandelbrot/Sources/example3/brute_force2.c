/* example3/brute_force2.c
 *
 * Like example2/brute_force1.c but using Vampire FPU features:
 * - 3 operant instructions (that replace two 2 operant instructions)
 * - scratch registers e2-e6
 *
...
int main(void) {
	...
	for (y=0; y<resy; y++) {
		for (x=0; x<resx; x++) {
			cy = y*stepi+mini;
			cx = x*stepr+minr;	
		    i=IterateAsmVampire0(cx,cy,MaxIter);
    ...
    floatpart=(ULONG)(exectime*100000000);
    /* print exectime as integer (for gcc) */
    intpart=(ULONG)exectime;
