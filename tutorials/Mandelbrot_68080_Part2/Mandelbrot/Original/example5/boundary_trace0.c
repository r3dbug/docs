...
/*
 * Same program structure as in example3 but brute force algorithm has been replaced 
 * by (very efficient) boundary trace.
 *
...
int main(void) {
    ...
    for(p=0; p<resx*resy-1; ++p) {
        ...
        if(!(Done[p+1] & Loaded)) {
            Data[p+1]=Data[p];
            /* write pixel to saga buffer */
			*((UBYTE*)newbuffer+p+1)=Data[p];
			Done[p+1] |= Loaded;
        }
    }
	SetStop();
