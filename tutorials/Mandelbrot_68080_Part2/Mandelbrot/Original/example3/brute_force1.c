/*
 * example3/brute_force1.c
 *
 * Like example2/brute_force0.c but iteration part replace by classic 68881 assembly code
 *
 * A bit faster ... :)
 *
...
int main(void) {
	...
	SetStart();
	/* Mandelbrot brute force algorithm with classic asm iteration */
	for (y=0; y<resy; y++) {
		for (x=0; x<resx; x++) {
			cy = y*stepi+mini;
			cx = x*stepr+minr;	
